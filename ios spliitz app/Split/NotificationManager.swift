...
class NotificationManager: ObservableObject {
    ...
    func requestPermission() {
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .badge, .sound]) { granted, error in
            DispatchQueue.main.async {
                ...
                if granted {
                    DebugLogger.shared.log("Notification permission granted")
                } else if let error = error {
                    DebugLogger.shared.log("Notification permission error: \(error.localizedDescription)")
                }
            }
        }
    }
    ...
    func setupListeners(userId: UUID) async {
        ...
        DebugLogger.shared.log("Setting up realtime listeners for user: \(userId)")
        ...
        do {
            ...
            DebugLogger.shared.log("Error subscribing to friend requests: \(error)")
        }
        ...
        do {
            ...
            DebugLogger.shared.log("Error subscribing to group invites: \(error)")
        }
        ...
        do {
            ...
            DebugLogger.shared.log("Error subscribing to expenses: \(error)")
        }
        ...
        // 4. Activities Listener (For Profile Badge)
        let notifChannel = client.channel("activities")
        ...
        do {
            ...
            DebugLogger.shared.log("Error subscribing to activities: \(error)")
        }
        channels.append(notifChannel)
        
        let notifTask = Task {
            for await _ in notifStream {
                // Any change to activities (insert/update) refreshes the badge
    ...
    func fetchProfileBadge(userId: UUID) async {
        ...
        do {
            ...
            DebugLogger.shared.log("Error fetching profile badge: \(error)")
        }
    }
    
    private func fetchHomeBadge(userId: UUID) async {
        // Home badge is now driven by activities table, not upcoming expenses
        // Set to 0 - badges only come from activities table via Profile badge
        DispatchQueue.main.async {
    ...
    private func fetchFriendsBadge(userId: UUID) async {
        ...
        do {
            ...
            DebugLogger.shared.log("Error fetching friends badge: \(error)")
        }
    }
    ...
    private func fetchGroupsBadge(userId: UUID) async {
        do {
            ...
            DebugLogger.shared.log("DEBUG: Groups Badge - Invites: \(inviteCount), Proposals: \(proposalCount)")
            
            DispatchQueue.main.async {
                self.groupsBadgeCount = inviteCount + proposalCount
            }
        } catch {
            DebugLogger.shared.log("Error fetching groups badge: \(error)")
        }
    }
    ...
    func scheduleDeadlineNotification(for expense: ExpenseInfo) {
        ...
        UNUserNotificationCenter.current().add(request) { error in
            if let error = error {
                Task { @MainActor in
                    DebugLogger.shared.log("Error scheduling deadline: \(error)")
                }
            }
        }
    }
    ...
    func scheduleNotification(title: String, body: String) {
        ...
        UNUserNotificationCenter.current().add(request) { error in
            if let error = error {
                Task { @MainActor in
                    DebugLogger.shared.log("Error scheduling notification: \(error)")
                }
            }
        }
    }
    ...
    func scheduleTestNotification() {
        ...
        UNUserNotificationCenter.current().add(request) { error in
            Task { @MainActor in
                if let error = error {
                    DebugLogger.shared.log("Error scheduling test notification: \(error)")
                } else {
                    DebugLogger.shared.log("Test notification scheduled for 5 seconds from now")
                }
            }
        }
    }
