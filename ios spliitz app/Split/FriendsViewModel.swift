...
class FriendsViewModel: ObservableObject {
    ...
    func fetchFriends(force: Bool = false) async {
        ...
        do {
            ...
            DebugLogger.shared.log("Fetch friends error: \(error)")
        }
    ...
    func fetchPendingRequests(force: Bool = false) async {
        ...
        do {
            ...
            DebugLogger.shared.log("Failed to fetch pending requests: \(error)")
        }
    }
    ...
    func searchUsers() async {
        ...
        do {
            ...
            DebugLogger.shared.log("Search failed: \(error)")
            searchResults = []
        }
    ...
    func respondToRequest(requesterId: UUID, accept: Bool) async {
        ...
        do {
            DebugLogger.shared.log("DEBUG: responding to request from \(requesterId), accept: \(accept)")
            try await service.respondToFriendRequest(requesterId: requesterId, requesteeId: currentUserId, accept: accept)
            DebugLogger.shared.log("DEBUG: respond success. Refreshing...")
            ...
            DebugLogger.shared.log("Respond request failed: \(error)")
