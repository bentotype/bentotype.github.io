...
struct GroupDetailView: View {
    ...
    @State private var expenseToEdit: ExpenseInfo?
    ...
    var body: some View {
        ScrollView {
                                    ...
                                    Section(header: Text(formatDateHeader(date))
                                        ...
                                        ForEach(expenses) { expense in
                                            HStack {
                                                if isEditing {
                                                    Button(action: {
                                                        Task { await viewModel.deleteExpense(expense: expense) }
                                                    }) {
            ...
            .padding() // Add padding to the container for consistent margins
            .frame(maxWidth: .infinity) // Ensure VStack takes full width but follows spacing
        }
        .appBackground() // Ensure background covers ScrollView
            ...
            .sheet(isPresented: $viewModel.showingCreateExpenseSheet) {
                CreateExpenseView(
                    ...
                    onComplete: {
                        ...
                        viewModel.scannedExplanation = nil
                        viewModel.scannedSplits = nil
                        expenseToEdit = nil // Clear edit state
                    }
                )
            }
            .onChange(of: expenseToEdit) { oldValue, newValue in
                if newValue != nil {
                    viewModel.showingCreateExpenseSheet = true
                }
            }
    ...
    private var groupedExpenses: [(Date, [ExpenseInfo])] {
        let grouped = Dictionary(grouping: viewModel.expenseActivity) { expense in
