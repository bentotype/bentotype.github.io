    ...
    func handleAppRefresh(task: BGAppRefreshTask) {
        ...
        let backgroundTask = Task {
            ...
            if let userId = SupabaseManager.shared.auth.currentUser?.id {
                ...
                if let activities = try? await SupabaseService.shared.fetchActivities(userId: userId) {
                    let count = activities.filter { !$0.isRead }.count
                    if count > 0 {
                        // 2. Trigger Local Notification
                        let content = UNMutableNotificationContent()
                        content.title = "New Activity"
                        content.body = "You have \(count) new items."
