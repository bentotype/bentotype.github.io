...
struct GroupsView: View {
    ...
    var sortedGroups: [GroupWithInfo] {
        if sortAlphabetically {
            return viewModel.groups.sorted { ($0.groupInfo?.title ?? "").localizedCaseInsensitiveCompare($1.groupInfo?.title ?? "") == .orderedAscending }
        }
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            if viewModel.isLoading {
                                ...
                                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                                    ForEach(sortedGroups) { groupWithInfo in
