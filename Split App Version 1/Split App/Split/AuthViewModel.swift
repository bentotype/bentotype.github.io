...
class AuthViewModel: ObservableObject {
        ...
        Task {
            ...
            do {
                ...
                if response.session == nil {
                    errorMessage = "Sign-up requires email confirmation. Check your inbox."
                    isLoading = false
                    return
                }
    ...
    func changePassword(oldPassword: String, newPassword: String, confirmPassword: String) async throws {
        guard !oldPassword.isEmpty, !newPassword.isEmpty, !confirmPassword.isEmpty else {
            throw NSError(domain: "Auth", code: -1, userInfo: [NSLocalizedDescriptionKey: "Please fill out all password fields"])
        }
        
        guard newPassword.count >= 6 else {
            throw NSError(domain: "Auth", code: -1, userInfo: [NSLocalizedDescriptionKey: "New password must be at least 6 characters"])
        }
        
        guard newPassword == confirmPassword else {
            throw NSError(domain: "Auth", code: -1, userInfo: [NSLocalizedDescriptionKey: "New passwords do not match"])
        }
        
        guard newPassword != oldPassword else {
            throw NSError(domain: "Auth", code: -1, userInfo: [NSLocalizedDescriptionKey: "New password must be different from old password"])
        }
        ...
        do {
            ...
            let session = try await withThrowingTaskGroup(of: Session?.self) { group in
                ...
                group.addTask {
                    ...
                    throw NSError(domain: "Auth", code: -1, userInfo: [NSLocalizedDescriptionKey: "Session check timed out"])
                }
