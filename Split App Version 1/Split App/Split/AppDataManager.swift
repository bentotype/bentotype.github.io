...
class AppDataManager: ObservableObject {
    ...
    func preloadAllData() async {
        ...
        async let notificationsTask = service.fetchNotifications(userId: userId)
        
        do {
            let (friendsResult, requestsResult, groupsResult, invitesResult, userResult, totalResult, notificationsResult) = try await (
                friendsTask,
                requestsTask,
                groupsTask,
                invitesTask,
            ...
            self.notifications = notificationsResult
            
            print("AppDataManager: All data preloaded successfully")
        } catch {
            print("AppDataManager: Preload error - \(error)")
