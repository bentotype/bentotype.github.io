...
import { getUserInfo, canJoinGroups, canUseCamera } from '../users.js';
import { fetchUserGroups, fetchGroupInvites, fetchGroupMembers, fetchGroupPendingExpenses, fetchGroupExpenseActivity } from '../fetchers.js';
...
export async function renderReceiptUploadPage() {
  const user = appState.currentUser;
  if (!user) {
    appState.currentView = 'auth';
    render();
...
export async function renderExpenseInfoPage() {
  const user = appState.currentUser;
  if (!user) {
    appState.currentView = 'auth';
    render();
  ...
  // Initialize expense items from receipt if empty
  if (appState.pendingReceiptItems && appState.pendingReceiptItems.length > 0 && appState.expenseItems.length === 0) {
    appState.expenseItems = appState.pendingReceiptItems.map(i => ({
      id: crypto.randomUUID(),
      name: i.name || '',
      price: i.price || 0
    }));
    if (appState.expenseItems.length > 0) {
      appState.isItemizedMode = true;
    }
  }
  ...
  const itemsListMarkup = appState.expenseItems.map(item => `
        <div class="expense-item-row" data-item-id="${item.id}">
            <input class="expense-item-name" placeholder="Item name" value="${escapeHtml(item.name)}" data-action="update-item-name" />
  ...
  const totalFromItems = appState.expenseItems.reduce((sum, item) => sum + (Number(item.price) || 0), 0).toFixed(2);
  const finalTotal = appState.isItemizedMode ? totalFromItems : totalPrefill;
        ...
        <div class="expense-field">
            <div class="flex justify-between items-center mb-2">
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    <span class="text-sm text-gray-500">Itemized</span>
                    <input type="checkbox" id="itemized-mode-toggle" class="toggle-checkbox" ${appState.isItemizedMode ? 'checked' : ''} data-action="toggle-itemized-mode">
                </label>
            </div>
            
            <div id="itemized-list-container" class="${appState.isItemizedMode ? '' : 'hidden'} space-y-2 mb-3">
            ...
            <label class="expense-field">
              ...
              <input name="total_amount" type="number" step="0.01" min="0" required data-expense-total inputmode="decimal" class="expense-input ${appState.isItemizedMode ? 'bg-gray-100' : ''}" placeholder="0.00" value="${finalTotal}" ${appState.isItemizedMode ? 'readonly' : ''} />
            </label>
        </div>
...
export function buildExpenseMemberMarkup() {
  const members =
